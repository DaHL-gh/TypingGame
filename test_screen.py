...
class Test(Root):
    ...
    def build(self) -> None:
        ...
        LineLayout(parent=al, size_hint=(1, 1), program=ProgramManager(self.ctx).get('background_fog'), id='ll')

        self.root.gui.animation_manager.add(self._get_animation())

    def _get_animation(self) -> Animation:
        return Animation(id='back', func=self._animation_func, start=pg.time.get_ticks())

    def _animation_func(self, start: int, time: int, end: int):
        self.ll._vao.program['time'].write(glm.vec1(time / 1000))
        self.ll.redraw_request()
